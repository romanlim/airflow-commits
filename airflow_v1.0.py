...
# # ### Count the number 'commit.verification.signature' group by commit.author.name
# =============================================================================
top_five_committers = result_df.groupby(['commit.author.name', 'commit.author.email'])['commit.verification.signature'].count().reset_index().sort_values('commit.verification.signature', ascending = False)[:top_x]
top_five_committers.rename(columns = {'commit.verification.signature': 'count'}, inplace=True)
print(top_five_committers) 

# =============================================================================
# # #### Q2: For the ingested commits, determine the committer with the longest commit streak.
# # ### Get min and max of commit date group by commit.author.name'
# =============================================================================
longest_commit_streak = result_df.groupby(['commit.author.name']).agg(earliest_commit_date = ('commit.committer.date', np.min), latest_commit_date = ('commit.committer.date', np.max))
...
# # ### Get the week day and hour from committed date, count number of 'commit.verification.signature'
# =============================================================================
commit_df = result_df[['commit.author.name', 'commit.author.email','commit.verification.signature', 'commit.committer.date']].copy()
...
hm_df = commit_df.groupby(['commit_weekday','commit_hour_block']).agg(count = ('commit.author.name', 'count')).reset_index()
